...
e_msg(pred_used_in(P,Q)) --> 
  "Predicate ",uscntoken(tid(P))," still used in ",uscntoken(tid(Q)).

e_msg(type_mismatch(E,T)) --> 
  {uparform(E,L,[])}, uscntokens(L), " is not ", e_type(T).
e_msg(too_few_arguments(E)) --> "Too few arguments in ", 
  {uparform(E,L,[])}, uscntokens(L).
e_msg(too_many_arguments(E)) --> "Too many arguments in ", 
  {uparform(E,L,[])}, uscntokens(L).
