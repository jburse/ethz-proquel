...
:- dcg scanmant(*int). 		/* scan digits of mantissa and eventually fraction */
:- dcg scanfrac(*int). 		/* scan digits of fraction */
...
:- dcg parschk(token). 		/* check for current token */
:- dcg parsfin. 		/* check for end of stream */

parserr(E,L,L) :- err(pars(E,L)).

parschk(X,[X|L],L) :- !.
parschk(X,L,L) :- err(pars(token_expect(X),L)).
...
parsprim(all(V,A))		--> [tall], !, parschk(tvar(V)), parsprim(A).
parsprim(ex(V,A))		--> [tex], !, parschk(tvar(V)), parsprim(A).
parsprim(true)			--> [ttrue], !.
...
parsfaktor(int(X))		--> [tint],!,parschk(tlpar),parsxpr(X),parschk(trpar).
parsfaktor(float(X))		--> [tfloat],!,parschk(tlpar),parsxpr(X),parschk(trpar).
parsfaktor(A)			--> [tlpar], !, parsform(A), parschk(trpar).
parsfaktor(pred(P,A))		--> [tid(P)], !, parsarg(A).
parsfaktor(_)			--> parserr(faktor_expect).

parsarg([A|B])			--> [tlpar], !, parsxpr(A), parsarg2(B), parschk(trpar).
parsarg([])			--> [].
parsarg2([A|B])			--> [tcomma], !, parsxpr(A), parsarg2(B).
parsarg2([])			--> [].

:- type etype = n(ntype) + s. 		/* expression types */

:- type ntype = i + f. 			/* numeric expression types */

:- type com = verbose + create(str,*etype) + 
           drop(str) + clear(str) + assert(str,*form,form) + 
...
:- dcg parsdecl(str,*etype).		/* parse declaration */
:- dcg parsdecl2(*etype).		/* parse middle part of declaration */
:- dcg parsdecl3(*etype).		/* parse rest of declaration */
:- dcg parstype(etype).			/* parse type */

parsrule(P,A,B)		--> parschk(tid(P)), parsarg(A), parsrule2(B).
parsrule2(Q)		--> [tlimp], !, parsform(Q).
parsrule2(true)		--> [].

parsdecl(P,A)		--> parschk(tid(P)), parsdecl2(A).
parsdecl2([A|B])	--> [tlpar], !, parstype(A), parsdecl3(B), parschk(trpar).
parsdecl2([])		--> [].
parsdecl3([A|B])	--> [tcomma], !, parstype(A), parsdecl3(B).
parsdecl3([])		--> [].
...
parscom(create(P,A))	--> [tcreate], !, parsdecl(P,A).
parscom(drop(P))	--> [tdrop], !, parschk(tid(P)), parsfin.
parscom(clear(P))	--> [tclear], !, parschk(tid(P)), parsfin.
...
:- dcg upardecl(str,*etype).		/* unparse declaration */
:- dcg upardecl2(*etype).		/* unparse middle of declaration */
:- dcg upardecl3(*etype).		/* unparse rest of declaration */
...
upardecl(P,L)			--> [tid(P)], upardecl2(L).
upardecl2([X|Y])		--> [tlpar], upartype(X), upardecl3(Y), [trpar].
upardecl2([])			--> [].
upardecl3([X|Y])		--> [tcomma], upartype(X), upardecl3(Y).
upardecl3([])			--> [].
