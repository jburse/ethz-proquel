...
(** Score: 20/20
  ...
  Nice and creative work! Full score!

  * Extends while-language with multi-dim arrays.
  * Independent from that, extends the terminating loop pattern homework
    by exit-conditions
  * Uses both to implement a lastIndexOf - function on 1-dim arrays,
    and show its termination. BUT NOT CORRECTNESS.
  * Specialities: The state is modeled as a total function, mapping each
    array\<times>indices to a value. That is, arrays do neither have a length,
    and each variable name names one distinct array per dimension.

  The formalization is well-documented.  
*)
...
(** On firts glance, I did not understand the modeling of arrays in the state.
  Why do you map pairs if names and lists of values to integers.
  
  I would have expected something like "vname \<Rightarrow> val list", or
  perhaps vname \<Rightarrow> val + val list, if you want to distinguish between
  array and non-array values.

  OK. I assume you are modeling multi-dimensional arrays,
  and the state says how to map each index (a list of vals) to its
  value. This is a valid modeling!
*)
