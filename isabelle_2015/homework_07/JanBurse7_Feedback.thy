...
(**
  Isar and Isabelle *is* a very complex framework, and it requires a lot 
  of practice to become expert. But the learning curve is quite 
  steep initially, so you actually can do many proofs only with a basic
  knowledge of HOL and Isar.

  To fully understand how such proofs work, you should not rely on automatic 
  proof setup ([intro,dest,simp, ...] - attributes) too much, because they tend
  to perform crucial steps automatically in some cases, and are not very robust
  to modifications, i.e., after changing some little bit, the proofs do not work, and
  from the proof you cannot see how it used to work anymore, because all stuff was done
  implicitly by intro/dest/simp rule setup.
*)
